...
export const results = createModel({
  ...
  reducers: {
    ...
    prepareBatch (state, { firstNotLoaded, pageSize }) {
      const totalCount = state.resultMetadata?.length || 0
      if (totalCount > firstNotLoaded) {
        const elementsToInit = Math.min(pageSize, totalCount)

        state.array.splice(firstNotLoaded, elementsToInit, ...new Array(elementsToInit).map(() => ({})))
      }

      return { ...state }
    },
    populateBatch (state, { firstNotLoaded, subjects, graph }) {
  ...
  effects (store) {
    ...
    return {
      ...
      async load (from = 0) {
        ...
        const subjects = sparqlList.resultSubjects(dataset, order.descending)

        dispatch.results.prepareBatch({
          firstNotLoaded,
          pageSize: Math.min(pageSize, subjects.length)
        })
