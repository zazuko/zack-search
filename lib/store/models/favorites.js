    ...
    return {
      addOrOrRemoveSubject ({ list, subject }) {
        ...
        dispatch.core.showMessage({
          message: translate.get('zack-starred-dialog.item-starred'),
          action: {
            text: translate.get('zack-starred-dialog.add-note'),
            callback () {
      ...
      addListFilter ({ list }) {
        ...
        dispatch.search.setFilter({
          id: 'list',
          label: `${translate.get('zack-filter-summary.list-prefix')}: ${label}`,
          list,
