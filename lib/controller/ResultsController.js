...
export class ResultsController {
  ...
  async load (from = 0) {
    ...
    const dataset = await this.sparqlList.fetchResults(firstNotLoaded, store.state.search.fetchAll)
    const subjects = this.sparqlList.resultSubjects(dataset)
  ...
  order () {
    const email = store.state.core.templates['records-order']
    if (!email) {
      return alert('Missing template[id=records-order]')
    }

    const recordDetailProperties = getProperties(store.state.core.shapes['order-shape'])

    const records = this.array.filter(row => !!row).map(({ pointer }) => pointer)
    const table = html`<ul>
      ${records.map(record => html`<li>
        ${record.value}
        <ul>
          ${recordDetailProperties.map(({ path }) => html`<li>
            ${findNodes(record, path).values}
          </li>`)}
        </ul>
      </li>`)}
    </ul>`
    ...
    render(html`${email.body.cloneNode(true)}\n${table}`, body)

    location.href = `mailto:${email.to}?body=${body.innerHTML}&subject=${email.subject}`
  }
}
