...
export class ResultsController {
  ...
  order () {
    ...
    const records = this.array.reduce((rows, row) => {
      if (!row) return rows
      return [...rows, [row.subject.value]]
    }, [])
    const table = markdownTable([['Record'], ...records])

    const body = `${email.body}

${table}`.replaceAll('\n', '%0D%0A').replaceAll(' ', ' ')

    location.href = `mailto:${email.to}?body=${body}&subject=${email.subject}`
  }
}
