...
class ZackResults extends connect(store, ZackComponent) {
  ...
  async updated (_changedProperties) {
    ...
    if (_changedProperties.has('shapePointer') && this.shapePointer) {
      this.shape = this.__transformShape(this.shapePointer)
...
${e.stack}
  ...
  render () {
    return html`
      ...
      <div id="buttons" ?hidden="${!this.selectedList}">
        <button @click="${() => this.results.download(this.selectedList)}">Export</button>
        <button @click="${() => this.results.order(this.selectedList)}">Order</button>
      </div>`
  }
  ...
  listScrolled (e) {
    this.results.load(e.first)
  }

  mapState (state) {
    return {
      shapePointer: state.core.shapes.result,
      selectedList: state.search.listFilter?.list
    }
  }

  __transformShape (shape) {
    if (!shape) {
      return null
    }
