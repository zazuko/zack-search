...
class ZackResult extends ZackComponent {
  ...
  async updated (_changedProperties) {
    if (!((_changedProperties.has('shapes') || _changedProperties.has('subject')) && this.subject && this.shapes)) {
      return
    }

    const subjectTypes = this.subject.out(rdf.type)
    const shape = this.shapes.find(shape => shape.pointer.has(sh.targetClass, subjectTypes).terms.length)
    ...
    this.properties = [...Object.entries(shape.properties)]
  ...
  render () {
    ...
    return html`
      ...
      ${Object.values(this.parts).map(part => html`<div part="${this.compact ? 'compact' : ''} ${part.name}">
