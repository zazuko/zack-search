...
class ZackResultHierarchy extends ZackComponent {
    ...
    for (lvl in hierarchy.slice(0, -1)) {
      ...
      <li>
        ...
        <a data-toggle="tooltip" data-placement="bottom"
           style="background-color: var(--zack-level-${lvlShort}-color, ${lvlColor})"
        > 
          <span class="filter-title" title="${hierarchy[lvl].title}">${hierarchy[lvl].title}</span>
          <span class="result-hierarchy-after" style="border-left-color: var(--zack-level-${lvlShort}-color, ${lvlColor})"> </span>
        </a>
        </zack-filter>
      </li>`
    }
  ...
  getHierarchy (ptr) {
    let title = this.shape.properties.title.findNodes(ptr).values.shift()
    const level = this.shape.properties.level.findNodes(ptr).values.shift()
    ...
    const relation = this.shape.properties.relation.findNodes(ptr).toArray().shift()
