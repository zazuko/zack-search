...
class ZackListsDialog extends connect(store, LitElement) {
  ...
  render () {
    return html`<sp-dialog-wrapper headline="${translate('zack-lists-dialog.header')}" dismissable underlay>
      ${this.lists.map(list => html`<div>
        ${this.listLabels[list] || list} <sp-icon-delete @click="${this.__delete(list)}"></sp-icon-delete>
      </div>`)}
      <div>
        <sp-textfield placeholder="${translate('zack-lists-dialog.new-list-placeholder')}" @change="${e => store.dispatch.favorites.addList({ list: e.target.value })}"></sp-textfield>
        <sp-button size="s" variant="primary" @click="${this.__add}">${translate('zack-lists-dialog.add-list')}</sp-button>
      </div>
    </sp-dialog-wrapper>`
  }
