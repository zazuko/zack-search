...
class ZackSearch extends connect(store, ZackComponent) {
  ...
  constructor () {
    ...
    this.__initSort = once(() => {
      customElements.whenDefined('zack-query-order').then(() => {
        this.querySelector('zack-query-order[selected]')?.sort()
      })
    })
  }
  ...
  __updateOrderElements (order) {
    for (const zackOrder of this.querySelectorAll('zack-query-order')) {
      zackOrder.selected = order.id === zackOrder.id
      zackOrder.descending = !!order.descending
    }
  }

  mapState (state) {
    if (state.search.order?.id) {
      this.__updateOrderElements(state.search.order)
    } else {
      this.__initSort()
    }
