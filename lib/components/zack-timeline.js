...
class ZackTimeline extends ZackComponent {
  ...
  firstUpdated () {
    ...
    this.fromHandle = d3.select(this.renderRoot).select('.handle--w')
      .attr('id', 'from-handle')
      .attr('data-filter', '>=')
      .attr('data-predicate', 'http://www.w3.org/2006/time#intervalStarts')

    this.toHandle = d3.select(this.renderRoot).select('.handle--e')
      .attr('id', 'to-handle')
      .attr('data-filter', '<=')
      .attr('data-predicate', 'http://www.w3.org/2006/time#intervalStarts')

    this.dispatchEvent(new CustomEvent('zack-html-filters', {
      bubbles: true,
      composed: true,
      detail: {
        elements: [this.toHandle.node(), this.fromHandle.node()]
      }
    }))
