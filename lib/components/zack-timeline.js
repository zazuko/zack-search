...
class ZackTimeline extends connect(store, ZackComponent) {
  ...
  async connectedCallback () {
    ...
    if (!RO) {
      RO = (await import('resize-observer')).ResizeObserver
    }

    this.resizeObserver = new RO(debounce(this._render.bind(this), 500))
  ...
  _render () {
    ...
    const query = this.buildQuery({
      ...
      width: this.renderRoot.querySelector('#timeline-container').width.baseVal.value - this.margin.left - this.margin.right
    })
    ...
    const start = this.resultMetadata?.start || new Date()
    const end = this.resultMetadata?.end || new Date()
    ...
    const zoom = () => {
      ...
      // update filters
      const newFromDate = new Date(that.x.domain()[0].toDateString())
      const newToDate = new Date(that.x.domain()[1].toDateString())

      store.dispatch.search.setFilter({
        ...this.__createFromDateFilter(toRdf(newFromDate)),
        id: 'from',
        label: `> ${format('yyyy-MM-dd', newFromDate)}`
      })
      store.dispatch.search.setFilter({
        ...this.__createToDateFilter(toRdf(newToDate)),
        id: 'to',
        label: `< ${format('yyyy-MM-dd', newToDate)}`
      })
    }
  ...
  __createFromDateFilter (value) {
    const patternOverride = this.querySelector('.start-query-pattern')
    if (patternOverride) {
      return {
        expression: patternOverride.textContent,
        value
      }
    }
    ...
    return {
      operator: '>=',
      predicate: time.intervalStarts,
      value
    }
  }

  __createToDateFilter (value) {
    const patternOverride = this.querySelector('.end-query-pattern')
    if (patternOverride) {
      return {
        expression: patternOverride.textContent,
        value
