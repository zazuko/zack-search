...
class ZackTimeline extends connect(store, ZackComponent) {
  ...
  _render () {
    ...
    const zoom = () => {
      ...
      // update filters
      const newFromDate = toRdf(that.x.domain()[0])
      const newToDate = toRdf(that.x.domain()[1])

      store.dispatch.search.setFilter({
        ...this.__createFromDateFilter(newFromDate),
        id: 'from',
        display: false
      })
      store.dispatch.search.setFilter({
        ...this.__createToDateFilter(newToDate),
        id: 'to',
        display: false
      })
    }
