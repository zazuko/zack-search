...
renderer.renderResult = function (page, subject) {
  ...
  if (title) {
    ...
    console.log('Missing title ', subject.toString())
  }

  var titleLink = '<a href="' + subject.toString() + '">' + titleString + '</a>'

  var faLevel = renderer.options.levels
  ...
  var level = '<div class="result-level-wrap"><div class="vertical-text result-level" data-filterable="="' +
      ' data-toggle="tooltip" data-placement="right" title="Filter ' + levelShort + ': ' + subject + '"' +
      ' data-predicate="http://purl.org/dc/terms/hasPart"' +
      ' data-property-path-prefix="^"' +
      ' data-property-path-postfix="+"' +
      ...
      ' data-value="' + subject + '" ' +
      ' data-named-node' +
      ' onclick="app.search.addFilter(this)" style="background-color: ' + levelColor + '"><i class="fa '+ faLevel[levelShort].icon + '"></i></div></div>'
...
//  }
  ...
  rendering = '<div class="zack-result row">' +
      ...
      '<div class="col-lg-8 col-md-6 col-sm-6 col-xs-12">'+ reference + timeRange + '</div>' +
      '<div class="col-lg-4 col-md-6 hidden-sm hidden-xs">' + conceptTagDivs + '</div>' +
      '<div class="col-lg-4 hidden-md hidden-sm hidden-xs">' + noteSpan + physical + urlLink + '</div>' +
      '</div>'
