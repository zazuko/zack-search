...
renderer.renderResult = function (page, subject) {
  ...
  var faLevel = app.options.plugins.find(function(x) {return x.name == "TypeFilter"}).options.values

  var level = page.match(subject, 'http://data.archiveshub.ac.uk/def/level').toArray().shift()

  var levelString = level.object.toString()
  ...
  var level = '<div class="result-level-wrap"><div class="vertical-text result-level" data-filterable="="' +
      ...
      ' data-value="' + subject + '" ' +
      ' data-named-node' +
      ' onclick="app.search.addFilter(this)" style="background-color: ' + levelColor + '"><i class="fa '+ faLevel[levelString].icon + '"></i></div></div>'
