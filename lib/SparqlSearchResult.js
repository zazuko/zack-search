...
SparqlSearchResultList.prototype.buildMetadataQuery = function () {
  return this.buildMetadataFilterQuery().replace(/\${searchString}/g, store.state.search.textQuery)
}
...
SparqlSearchResultList.prototype.buildResultQuery = function (offset) {
  return this.buildResultFilterQuery()
    .replace('${searchString}', store.state.search.textQuery) // eslint-disable-line no-template-curly-in-string
...
SparqlSearchResultList.prototype.resultSubjects = function (page) {
  ...
  if (page.match(null, terms.score).length > 0) {
    subjects = subjects.sort(function (a, b) {
      const quadA = page.match(a, terms.score).toArray().shift()
      const quadB = page.match(b, terms.score).toArray().shift()

      const scoreA = quadA ? parseFloat(quadA.object.value) : 0
      const scoreB = quadB ? parseFloat(quadB.object.value) : 0
